...
/* forward declaration */
static void hpnsTransform(UINT32 * buf, UINT32 * in);
 ...
 */
void hpnsMD5Init(MD5_CTX * mdContext)
{
   ...
   )
{
   ...
   while (inLen--)
      ...
      if (mdi == 0x40)
         ...
         hpnsTransform (mdContext->buf, in);
         mdi = 0;
         }
      }
}
...
*/
void hpnsMD5Final(MD5_CTX *mdContext)
{
   UINT32 in[16];
   int mdi;
   unsigned int i, ii;
   ...
   hpnsMD5Update(mdContext, PADDING, padLen);
...
/* Basic MD5 step. hpnsTransforms buf based on in.
 */
static void hpnsTransform(UINT32 * buf, UINT32 * in)
{
...
int hpnsAuthenticateMsg(UINT8 *pMsg, int msgLen, UINT8 *pAuth, UINT8 *pKey) 
{
	...
	hpnsMD5Init (&context);
	hpnsMD5Update (&context, pKey, HPNS_KEY_LEN );
	hpnsMD5Update (&context, pMsg, msgLen);
	hpnsMD5Update (&context, pKey, HPNS_KEY_LEN ); 
	hpnsMD5Final (&context);
		 
	if ( memcmp(context.digest, pAuth, sizeof(context.digest)) == 0 ) 
		ret = 0;
...
int hpnsBuildAuthHeader(UINT8 *pMsg, int msgLen, UINT8 *pAuth, UINT8 *pKey)
{
	...
	hpnsMD5Init (&context);
	hpnsMD5Update (&context, pKey, HPNS_KEY_LEN );
	hpnsMD5Update (&context, (UINT8 *)pMsg, msgLen);
	hpnsMD5Update (&context, pKey, HPNS_KEY_LEN ); 
	hpnsMD5Final (&context); 
